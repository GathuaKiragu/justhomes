...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _handleGoogleSignIn(BuildContext context) async {
    ...
    try {
      ...
      if (user != null) {
        // Extract providerId and prepare data
        final providerId = user.id; // Unique to the Google account
        ...
        // Send data to the backend for social media login
        var res = await CallApi().postData(data, 'user/social-media-login');
        var body = json.decode(res.body);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Sign in aborted by the user'),
          ),
        );
      }
      ...
      // Handle specific PlatformException
      if (e.code == 'sign_in_failed') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            backgroundColor: Colors.red,
            content: Text(
                'Google Sign-In failed. Please check your configuration or try again later.'),
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
